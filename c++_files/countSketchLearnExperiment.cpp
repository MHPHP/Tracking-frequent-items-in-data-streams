...
//    }
        ...
        for (float delta: delta_vec) {
            for (float eps: epsilon_vec) {
                for (float threshold: threshold_vec) {
                    ...
                    countSketchLearn cSketchLearn(eps, delta, threshold);
                    ...
                    for (auto const& query : query_predict_vector) {
                        ...
                        cSketchLearn.update(query_dict.at(get<0>(query)), 1,  get<1>(query));
                    }
                    ...
                    for (int i = 1; i <= query_dict.size(); i++) {
                        cSketchLearn.estimate(i,threshold_dict[i]);
                    ...
                    cout << "The Memory used in bytes is: " << cSketchLearn.getSize() << endl;
                    ...
                    for (int i = 1; i <= query_size; i++) {
                        ...
                        uint64_t fHat = cSketchLearn.estimate(i, threshold_dict[i]);
                    ...
                    memory_used.push_back(cSketchLearn.getSize());
    ...
    output_file.open ("../../ResultData/countSketchLearnSketchConstantDelta_11.csv");
